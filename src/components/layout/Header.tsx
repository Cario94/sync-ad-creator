...
const Header = () => {
  ...
  const scrollToSection = (hash: string) => {
    // If already on /, scroll directly; otherwise navigate first
    if (location.pathname === '/') {
      const el = document.getElementById(hash);
      el?.scrollIntoView({ behavior: 'smooth' });
    }
    // For off-page, the Link will navigate to /#hash and Index will handle it
  };
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <Link to="/" className="text-2xl font-bold text-gradient" aria-label="CampaignSync Home">
          CampaignSync
        </Link>
        ...
        <nav className="hidden md:flex items-center space-x-8">
          ...
          <div className="flex items-center space-x-4">
            {session ? (
              ...
              <>
                <Link to="/login">
                  <Button variant="outline" className="font-medium px-6">Login</Button>
                </Link>
                <Link to="/register">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 glass-morphism animate-fade-down py-6 px-6">
          <nav className="flex flex-col space-y-4">
            <Link to="/" className="font-medium hover:text-primary transition-colors py-2">Home</Link>
            <Link
              to="/#features"
              ...
              {session ? (
                ...
                <>
                  ...
                  <Link to="/register" className="w-full">
                    <Button className="font-medium w-full">Get Started</Button>
                  </Link>
                </>
              )}
            </div>
